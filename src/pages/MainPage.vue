...
const startScrollAnimation = <T extends ElementAndNull>(refNode: T) => {
    ...
    const getNextNode = () => {
        ...
        state.nodeArrLength = nodeArr.length
    ...
    const scrollAnimation = <T extends ElementAndNull>(refNode: T) => {
        if (refNode) {
            ...
            if (refNode.children.length <= 1) {
                ...
                const scrollHandler = () => {
                    if (refNode.scrollTop >= (refNode.scrollHeight - refNode.clientHeight)) {
                        refNode.removeEventListener('scroll', scrollHandler)
                ...
                refNode.addEventListener('scroll', scrollHandler)
...
const getDivination = () => {
    ...
    let lunarDayIndex = currentLunar.getDay()
    const lunarHourIndex = currentLunar.getTime().getZhiIndex() + 1

    //早子时算第二天
    if (currentSolar.getHour() >= 23 && currentLunar.getHour() < 24) {
        lunarDayIndex++
    }
